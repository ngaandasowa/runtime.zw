...
export const DashboardDomains: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-200 pb-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-extrabold text-zinc-950 tracking-tight flex items-center space-x-2">
            <Globe className="h-6 w-6 text-[#FF2D20]" />
        ...
        <div className="flex items-center space-x-2">
          <div className="inline-flex rounded-xl border border-zinc-200 bg-white p-1 text-xs font-bold shadow-2xs">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg transition ${activeTab === 'my_domains' ? 'bg-[#FF2D20] text-white' : 'text-zinc-600 hover:text-zinc-900'}`}
            >
              My Domains ({userDomains.length})
            </button>
            <button
              onClick={() => setActiveTab('transfers')}
              className={`px-3 py-1.5 rounded-lg transition ${activeTab === 'transfers' ? 'bg-[#FF2D20] text-white' : 'text-zinc-600 hover:text-zinc-900'}`}
            >
              Transfer In (T)
            </button>
          </div>
          ...
          <button
            ...
            className="inline-flex items-center space-x-1.5 rounded-xl bg-[#FF2D20] px-3.5 py-2 text-xs font-bold text-white hover:bg-[#E0241A] transition shadow-xs"
          >
          ...
          {userDomains.length === 0 ? (
            <div className="text-center py-12 border border-dashed border-zinc-200 rounded-2xl bg-white shadow-2xs">
              ...
              <p className="text-xs text-zinc-500 max-w-sm mx-auto mt-1 mb-4">
                Secure your Zimbabwean domain name today with direct ZISPA delegation and $2.00/yr pricing.
              </p>
              <button
                onClick={() => setRegistrationModalOpen(true)}
                className="inline-flex items-center space-x-2 rounded-xl bg-[#FF2D20] px-5 py-2.5 text-xs font-bold text-white hover:bg-[#E0241A] shadow-xs"
              >
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-zinc-500">
                        ...
                        <div>•</div>
                        <div>Renewal: <span className="text-[#FF2D20] font-bold">${domain.renewal_price.toFixed(2)}/yr</span></div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      >
                        <Server className="h-3.5 w-3.5 text-[#FF2D20]" />
                      ...
                      <button
                        ...
                        className="inline-flex items-center space-x-1 rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-1.5 text-xs font-semibold text-zinc-700 hover:bg-white hover:border-zinc-300 transition"
                      >
                        <Edit3 className="h-3.5 w-3.5 text-[#FF2D20]" />
                        <span>Modify (M)</span>
                      </button>
      ...
      {activeTab === 'transfers' && (
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 sm:p-8 max-w-2xl mx-auto shadow-sm">
          <div className="flex items-center space-x-3 mb-4">
            <ArrowLeftRight className="h-6 w-6 text-[#FF2D20]" />
            <div>
              <h2 className="text-lg font-bold text-zinc-950">Transfer Domain into Runtime</h2>
              <p className="text-xs text-zinc-500">Initiate an official ZISPA Transfer (Action T) to Ngaatec registrar infrastructure.</p>
            </div>
          </div>
          ...
          <form onSubmit={handleTransferSubmit} className="space-y-4 text-xs">
            <div>
              ...
              <input
                ...
                className="w-full rounded-xl border border-zinc-200 bg-zinc-50 p-3 text-sm text-zinc-900 focus:border-[#FF2D20] focus:bg-white focus:outline-none font-mono"
              />
            </div>
            ...
            <div className="p-3 rounded-xl bg-zinc-50 border border-zinc-200 text-zinc-600">
              <strong className="text-zinc-900 font-bold">ZISPA Transfer Policy:</strong> Domain transfers do not incur extra transfer penalties. Once requested, a ZISPA Action T form is generated and dispatched to the registry.
            </div>

            <button
              type="submit"
              disabled={!transferDomain.trim()}
              className="w-full inline-flex items-center justify-center space-x-2 rounded-xl bg-[#FF2D20] py-3 text-sm font-bold text-white hover:bg-[#E0241A] transition disabled:opacity-50 shadow-sm"
            >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs">
          <div className="relative w-full max-w-lg rounded-2xl border border-zinc-200 bg-white p-6 text-zinc-900 shadow-2xl ring-1 ring-black/5">
            ...
            <p className="text-xs text-zinc-500 mb-4">
              Specify 2 to 4 authoritative nameservers. Updating queues a ZISPA MODIFY (M) request.
            </p>
              ...
              {editNs.map((ns, idx) => (
                <div key={idx}>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-zinc-200 bg-zinc-50 p-2 text-xs text-zinc-900 focus:border-[#FF2D20] focus:bg-white focus:outline-none"
                  />
                </div>
              ))}
      ...
      {/* MODAL 2: Modify Details (ZISPA M) */}
      {modalMode === 'modify' && selectedDomain && modifyOwner && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs overflow-y-auto">
          <div className="relative w-full max-w-xl rounded-2xl border border-zinc-200 bg-white p-6 text-zinc-900 shadow-2xl ring-1 ring-black/5 my-8">
            <h3 className="text-base font-bold text-zinc-950 mb-1">
              Modify Domain Contact (ZISPA Action M)
            </h3>
            <p className="text-xs text-zinc-500 mb-4">
              Target: <strong className="text-zinc-950 font-mono">{selectedDomain.domain_name}</strong>. ZISPA requires complete contact specifications.
            </p>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl bg-zinc-50 border border-zinc-200 p-2 text-zinc-900 focus:border-[#FF2D20] focus:bg-white focus:outline-none"
                  />
                </div>
                <div>
      ...
      {/* MODAL 3: Delete Protection (ZISPA D) */}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs">
          <div className="relative w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-6 text-zinc-900 shadow-2xl ring-1 ring-black/5">
            <div className="flex items-center space-x-2 text-zinc-950 mb-2">
              <ShieldAlert className="h-5 w-5 text-[#FF2D20]" />
              <h3 className="text-base font-bold">Delete Protection Warning</h3>
            </div>
            
            <p className="text-xs text-zinc-600 mb-3 leading-relaxed">
              You are requesting deletion of <strong className="text-zinc-950 font-mono">{selectedDomain.domain_name}</strong> from the official ZISPA registry.
            </p>
            ...
            <input
              ...
              className="w-full rounded-xl border border-zinc-200 bg-zinc-50 p-2.5 font-mono text-xs text-zinc-900 focus:border-[#FF2D20] focus:bg-white focus:outline-none mb-4"
            />
          ...
          <div className="relative w-full max-w-lg rounded-2xl border border-zinc-200 bg-white p-6 text-zinc-900 shadow-2xl ring-1 ring-black/5">
            ...
            <div className="space-y-4 max-h-72 overflow-y-auto pr-2">
              {selectedDomain.history.map((h, i) => (
                <div key={i} className="flex items-start space-x-3 border-l-2 border-[#FF2D20] pl-3 py-1 text-xs">
                  <div>
                    <div className="flex items-center space-x-2">
                      <span className="font-bold text-zinc-950">Action {h.action}</span>
                      <span className="text-[10px] text-[#FF2D20] bg-red-50 px-1.5 py-0.5 rounded font-semibold border border-red-200">{h.status}</span>
