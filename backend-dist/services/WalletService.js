...
const toMinorUnits = (value) => Math.round(normaliseMoney(value) * 100);
const fromMinorUnits = (value) => value / 100;
const makeLedgerId = (idempotencyKey) => {
    const digest = crypto
        .createHash('sha256')
...
export class WalletService {
    ...
    async mutate(input) {
        ...
        if (!idempotencyKey ||
            !referenceId) {
            throw new Error('Wallet mutation requires a reference and idempotency key.');
        }
        ...
        const ledgerRef = adminDb
            .collection('wallet_transactions')
            .doc(makeLedgerId(idempotencyKey));
        const now = new Date().toISOString();
        return adminDb.runTransaction(async (transaction) => {
            const [walletDoc, existingLedger,] = await Promise.all([
            ...
            if (walletCurrency !==
                currency) {
                throw new Error('Wallet currency does not match this transaction.');
            }
            const beforeMinor = toMinorUnits(Number(wallet?.balance ||
                0));
            const amountMinor = toMinorUnits(amount);
            const increasesBalance = input.type ===
                'credit' ||
                input.type ===
                    'refund';
            const signedMinor = increasesBalance
                ? amountMinor
                : -amountMinor;
            const afterMinor = beforeMinor +
                signedMinor;
            if (afterMinor < 0) {
                throw new Error('Insufficient Runtime Credit.');
            }
