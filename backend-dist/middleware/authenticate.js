...
const loadProfile = async (uid, fallbackName) => {
...
export const authenticateWithProfile = async (req, res, next) => {
    ...
    const fallbackName = String(decoded.name ||
        decoded.email ||
        'Runtime customer');
    try {
        const profile = await loadProfile(decoded.uid, fallbackName);
        runtimeReq.runtimeUser = {
            uid: decoded.uid,
            email: decoded.email || '',
            name: profile.name,
        ...
        console.error('Runtime user profile load failed:', error);
        return res.status(503).json({
            success: false,
            message: 'Runtime database is temporarily unavailable. Please try again shortly.',
        });
    }
};
// Useful after an admin changes a user's role/profile in the same process.
export const clearRuntimeUserProfileCache = (uid) => {
    if (uid) {
        profileCache.delete(uid);
        return;
    }
