...
const classifyPesePayStatus = (transaction) => {
    const status = String(transaction.transactionStatus ||
        '')
        .trim()
        .toUpperCase();
    const description = String(transaction
        .transactionStatusDescription ||
        '')
        .trim()
        .toUpperCase();
    const methodStatus = String(transaction
        .paymentMethodDetails
        ?.paymentMethodStatus ||
        '')
        .trim()
        .toUpperCase();
    const methodMessage = String(transaction
        .paymentMethodDetails
...
const normalisePesePayMethod = (value) => {
    ...
    const isInnBucks = code === 'PZW212' ||
        ...
        searchable.includes('inn bucks');
    const isOmari = searchable.includes("o'mari") ||
        searchable.includes('omari') ||
        searchable.includes('o mari');
    /*
     * Runtime keeps card details off our own UI. PesePay's
     * seamless API supports mobile-money methods with a phone
     * number, so those can stay inside Runtime. Everything else
     * safely falls back to PesePay's hosted redirect checkout.
     *
     * This is deliberately capability-based at Runtime's edge:
     * new/unknown PesePay methods still work via redirect instead
     * of being rejected or guessed as seamless.
     */
    const seamlessMobileMoney = isEcoCash || isInnBucks || isOmari;
    return {
        code,
        name: name || code,
        description: String(value?.description ??
            ...
            '').trim(),
        seamless: seamlessMobileMoney,
        requiresPhone: seamlessMobileMoney,
    };
};
...
const settlePesePayPayment = async (paymentId, options = {}) => {
    const paymentRef = adminDb
        .collection('payments')
        .doc(paymentId);
    let paymentDoc = null;
    const attempts = options.waitForProviderReference
        ? 5
        : 1;
    for (let attempt = 0; attempt < attempts; attempt += 1) {
        ...
        if (options.waitForProviderReference &&
            attempt < attempts - 1) {
    ...
    const providerInternalReference = String(providerTransaction
        .internalReference ||
        providerReference);
    const paymentState = classifyPesePayStatus(providerTransaction);
    const now = new Date().toISOString();
    /*
    ...
    await adminDb.runTransaction(async (transaction) => {
        ...
        transaction.set(paymentRef, {
            ...
            rejection_reason: paymentState ===
                'failed'
                ? providerDescription ||
                    `PesePay returned ${providerStatus || 'a failed status'}.`
                : null,
            failed_at: paymentState ===
                'failed'
                ? (freshPayment.data()
                    ?.failed_at ||
                    now)
...
router.all('/pesepay/result', async (req, res) => {
    try {
        ...
        if (paymentId) {
            ...
            await settlePesePayPayment(paymentId, {
                waitForProviderReference: true,
            });
        }
...
router.post('/pesepay/verify', authenticate, async (req, res) => {
    try {
        ...
        const result = await settlePesePayPayment(paymentId);
        return res.json({
            success: true,
            ...result,
        });
    ...
    try {
        ...
        const result = await adminDb.runTransaction(async (transaction) => {
            ...
            if (action ===
                 ...
                 */
                const requiresRegistryConfirmation = String(domain.processing_type ||
                    '')
                    .trim()
                    .toLowerCase() ===
                    'zispa' ||
                    domainName.endsWith('.co.zw');
                if (requiresRegistryConfirmation &&
                    String(domain.nameserver_change_status ||
                        '')
                        .trim()
                        .toLowerCase() ===
                        'pending_registry') {
                    const error = new Error('A nameserver change is already in progress. Your new nameservers will appear after the registry successfully completes the current request.');
                    error.statusCode = 409;
                    throw error;
                }
 ...
 */
router.post('/admin/domain-replacement', authenticate, async (req, res) => {
    try {
        const runtimeUser = req.runtimeUser;
        if (runtimeUser.role !==
            'super_admin') {
        ...
        const domainId = typeof req.body?.domainId ===
            ...
            ? req.body.domainId.trim()
            : '';
        const replacementDomainName = typeof req.body
        ...
        if (!domainId ||
            ...
            return res.status(400).json({
                success: false,
                message: 'Original domain and a valid replacement domain are required.',
            });
        }
        const oldDomainRef = adminDb
            .collection('domains')
            .doc(domainId);
        ...
        const result = await adminDb.runTransaction(async (transaction) => {
             ...
             */
            const oldDomainDoc = await transaction.get(oldDomainRef);
            if (!oldDomainDoc.exists) {
                throw new Error('Original domain not found.');
            }
            const oldDomain = oldDomainDoc.data();
            if (![
                'cancelled',
                'registry_rejected',
                'pending_registration',
            ...
            const orderId = String(oldDomain.order_id ||
                '').trim();
            if (!orderId) {
                throw new Error('The original domain is not linked to an order.');
            }
            const orderRef = adminDb
                .collection('orders')
                .doc(orderId);
            ...
            if (!orderDoc.exists) {
                throw new Error('The paid order linked to this domain was not found.');
            }
            const order = orderDoc.data();
            if (order.user_id !==
                oldDomain.user_id) {
                throw new Error('Domain and order customer do not match.');
            }
            ...
            const verifiedPayments = paymentSnapshot.docs
                ...
                'verified');
            const verifiedTotal = Math.round(verifiedPayments.reduce((total, payment) => total +
            ...
            const parts = replacementDomainName
                .split('.');
            const tld = replacementDomainName
                .endsWith('.co.zw')
                ? '.co.zw'
                : replacementDomainName
                    .endsWith('.org.zw')
                    ...
                    : replacementDomainName
                        ...
                        : parts.length > 1
                            ...
                            : '';
            if (oldDomain.tld &&
                String(oldDomain.tld).toLowerCase() !==
                    tld.toLowerCase()) {
                throw new Error('A no-charge replacement must use the same domain extension as the original paid domain.');
            }
            ...
            const nextItems = Array.isArray(order.items)
                ? order.items.map((item) => item.item_type ===
                    'domain_registration'
                    ? {
                        ...item,
                        reference_id: replacementDomainName,
                        description: `Domain registration: ${replacementDomainName}`,
                    }
                    : item)
                : [];
            /*
             ...
             */
            transaction.set(oldDomainRef, {
                status: 'replaced',
                registry_rejection_reason: reason,
                rejected_at: oldDomain.rejected_at ||
                    now,
                archived_at: now,
                replacement_domain_id: replacementRef.id,
                replaced_by_domain: replacementDomainName,
                replaced_at: now,
                updated_at: now,
                history: [
                    ...oldHistory,
                    {
                        id: `hist-${crypto.randomUUID()}`,
                        domain_id: oldDomainRef.id,
                        action: 'STATUS_CHANGE',
                        description: `Registry replacement: ${oldDomain.domain_name} was replaced by ${replacementDomainName}. Existing payment/order retained.`,
                        status: 'replaced',
                        actor: runtimeUser.email ||
                            runtimeUser.uid,
                        created_at: now,
            ...
            transaction.set(orderRef, {
                status: order.status ===
                    ...
                    : order.status,
                items: nextItems,
                fulfillment_domain_id: replacementRef.id,
                original_domain_id: oldDomainRef.id,
             ...
             */
            return {
                replacementDomain: newDomain,
                originalDomainId: oldDomainRef.id,
                orderId,
        ...
        console.error('Domain replacement error:', error);
        const message = error instanceof Error
            ? error.message
            : 'Unable to replace domain.';
        const status = message.includes('not found')
            ? 404
            : message.includes('Only a cancelled') ||
                message.includes('not fully paid') ||
                message.includes('already exists') ||
                message.includes('do not match') ||
                message.includes('same domain extension')
                ? 409
                : 500;
 ...
 */
router.post('/admin/manual/approve', authenticate, async (req, res) => {
    try {
        const runtimeUser = req.runtimeUser;
        if (runtimeUser.role !==
            'super_admin') {
        ...
        const paymentId = typeof req.body?.paymentId ===
            'string'
            ? req.body.paymentId.trim()
            : '';
        const transactionId = typeof req.body?.transactionId ===
            'string'
        ...
        if (payment.gateway !==
            ...
            return res.status(400).json({
                success: false,
                message: 'Only manual EcoCash USD payments can be approved manually.',
            });
        }
        if (payment.status ===
            'rejected' ||
        ...
        const status = message === 'Payment not found.'
            ? 404
            : message.includes('can no longer be marked paid')
                ? 409
                : 500;
        return res.status(status).json({
            success: false,
...
router.post('/admin/manual/reject', authenticate, async (req, res) => {
    try {
        const runtimeUser = req.runtimeUser;
        if (runtimeUser.role !==
            'super_admin') {
        ...
        const result = await adminDb.runTransaction(async (transaction) => {
            ...
            if (payment.gateway !==
                'ecocash_usd') {
                throw new Error('Only manual EcoCash USD payments can be rejected manually.');
            }
            if (payment.status ===
                'verified') {
        ...
        const status = message === 'Payment not found.'
            ...
            : message.includes('cannot be rejected') ||
                message.includes('Only manual EcoCash')
                ? 409
                : 500;
        return res.status(status).json({
            success: false,
 ...
 */
router.post('/order/ecocash', authenticate, async (req, res) => {
    try {
        ...
        console.error('EcoCash order payment error:', error);
        return res.status(500).json({
            success: false,
            message: 'Unable to prepare EcoCash payment.',
        });
    }
});
/*
 ...
 * ----------------------------------------------------------
 * Uses the same PesePay method capability rules as order
 * payments. Mobile-money methods stay seamless; unsupported
 * methods fall back to PesePay's hosted redirect checkout.
 */
router.post('/wallet/pesepay/initiate', authenticate, async (req, res) => {
    let paymentId = '';
    try {
        const runtimeUser = req.runtimeUser;
        const amount = Math.round((Number(req.body?.amount) + Number.EPSILON) * 100) / 100;
        const paymentMethodCode = typeof req.body?.paymentMethodCode === 'string'
            ? req.body.paymentMethodCode.trim()
            : '';
        const customerPhoneNumber = typeof req.body?.customerPhoneNumber === 'string'
        ...
        if (!paymentMethodCode) {
            return res.status(400).json({
                success: false,
                message: 'Choose a PesePay payment method.',
            });
        }
        const currency = 'USD';
        const availableMethods = await fetchPesePayMethods(currency);
        const selectedMethod = availableMethods.find((method) => method.code === paymentMethodCode);
        if (!selectedMethod) {
            return res.status(400).json({
                success: false,
        ...
        if (selectedMethod.requiresPhone && !customerPhoneNumber) {
            return res.status(400).json({
                success: false,
                message: `${selectedMethod.name} phone number is required.`,
            });
        }
        const customerEmail = String(runtimeUser.email || '').trim();
        if (!customerEmail) {
            return res.status(400).json({
                success: false,
        ...
        paymentId = paymentRef.id;
        const merchantReference = `RT-CREDIT-${paymentId.slice(0, 12).toUpperCase()}`;
        const now = new Date().toISOString();
        await paymentRef.set({
            ...
            provider_payment_flow: selectedMethod.seamless ? 'seamless' : 'redirect',
            status: 'pending',
            customer_confirmed_payment: false,
            created_at: now,
            updated_at: now,
        ...
        const { integrationKey, encryptionKey } = getPesePayCredentials();
        const apiBaseUrl = process.env.RUNTIME_API_URL || 'https://api.runtime.co.zw';
        const frontendUrl = process.env.RUNTIME_FRONTEND_URL || 'https://runtime.co.zw';
        ...
        const paymentBody = selectedMethod.seamless
            ? {
                ...
                merchantReference,
                reasonForPayment: 'Runtime Credit top-up',
                resultUrl,
                returnUrl,
        ...
        if (!response.ok || !responseBody?.payload) {
            await paymentRef.set({
                status: 'failed',
                rejection_reason: `PesePay initiation failed${response.ok ? '' : ` with HTTP ${response.status}`}.`,
                updated_at: new Date().toISOString(),
            }, { merge: true });
            return res.status(502).json({
                success: false,
                message: 'PesePay could not initiate the top-up payment.',
            });
        }
        ...
        if (!transaction.referenceNumber) {
            await paymentRef.set({
                status: 'failed',
                rejection_reason: 'PesePay response did not include a reference number.',
                updated_at: new Date().toISOString(),
            }, { merge: true });
            return res.status(502).json({
                success: false,
        ...
        await paymentRef.set({
            ...
            transaction_id: transaction.internalReference || transaction.referenceNumber,
            provider_status: transaction.transactionStatus || 'INITIATED',
        ...
        return res.json({
            ...
            transaction: {
                ...
                flow: selectedMethod.seamless ? 'seamless' : 'redirect',
                paymentMethodCode: selectedMethod.code,
                paymentMethodName: selectedMethod.name,
                metadataCode: transaction.metadataCode ??
                    transaction.code ??
                    transaction.transactionMetadata?.code ??
                    transaction.transactionMetadata?.paymentCode ??
                    null,
                metadataQrCode: transaction.metadataQrCode ??
                    transaction.qrCode ??
                    transaction.transactionMetadata?.qrCode ??
                    transaction.transactionMetadata?.qrcode ??
                    null,
                transactionMetadata: transaction.transactionMetadata ?? transaction.metadata ?? null,
            },
        });
    }
    catch (error) {
        console.error('PesePay Runtime Credit initiation error:', error);
        if (paymentId) {
            try {
                await adminDb.collection('payments').doc(paymentId).set({
                    status: 'failed',
                    rejection_reason: 'Unable to initiate PesePay top-up.',
                    updated_at: new Date().toISOString(),
                }, { merge: true });
            }
            catch (cleanupError) {
                console.error('Unable to clean up failed PesePay top-up:', cleanupError);
            }
        }
        return res.status(500).json({
            success: false,
            message: 'Unable to initiate PesePay top-up.',
        });
    }
});
/*
...
router.post('/pesepay/initiate', authenticate, async (req, res) => {
    ...
    try {
        ...
        if (order.status ===
            'payment_pending') {
            const previousAttempts = await adminDb
                .collection('payments')
                .where('order_id', '==', orderId)
                .get();
            const staleBatch = adminDb.batch();
            let hasStaleUpdates = false;
            for (const attempt of previousAttempts.docs) {
                const attemptData = attempt.data();
                if (attemptData.gateway ===
                    'pesepay' &&
                    (attemptData.status ===
                        'pending' ||
                        attemptData.status ===
                            'pending_verification')) {
                    staleBatch.set(attempt.ref, {
                        status: 'failed',
                        rejection_reason: 'Previous payment attempt was replaced by a new customer retry.',
        ...
        const previousPendingPayments = await adminDb
            .collection('payments')
            .where('order_id', '==', orderId)
            .get();
        const retryBatch = adminDb.batch();
        previousPendingPayments.docs
            ...
            if (data.status ===
                'pending') {
                retryBatch.set(doc.ref, {
                    status: 'failed',
                    rejection_reason: 'Replaced by a new payment attempt.',
                    updated_at: new Date()
                        .toISOString(),
                }, { merge: true });
            }
        ...
        return res.json({
            ...
            transaction: {
                ...
                metadataQrCode: transaction.metadataQrCode ??
                    ...
                    transaction.transactionMetadata?.qrcode ??
                    null,
                transactionMetadata: transaction.transactionMetadata ??
