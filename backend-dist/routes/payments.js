...
router.post('/wallet/pesepay/initiate', authenticate, async (req, res) => {
    ...
    try {
        const runtimeUser = req.runtimeUser;
        const amount = Number(req.body?.amount);
        const paymentMethodCode = typeof req.body?.paymentMethodCode ===
            'string'
            ? req.body.paymentMethodCode.trim()
            : '';
        const customerPhoneNumber = typeof req.body?.customerPhoneNumber ===
            'string'
        ...
        if (!paymentMethodCode) {
            return res.status(400).json({
                success: false,
                message: 'PesePay payment method is required.',
            });
        }
        const roundedAmount = Math.round((amount + Number.EPSILON) *
            100) / 100;
        ...
        const customerEmail = String(runtimeUser.email ||
            '').trim();
        if (!customerEmail) {
            return res.status(400).json({
                success: false,
        ...
        for (const attempt of previousAttempts.docs) {
            ...
            if (data.purpose ===
                ...
                staleBatch.set(attempt.ref, {
                    status: 'failed',
                    rejection_reason: 'Previous Runtime Credit top-up was replaced by a new payment attempt.',
        ...
        paymentId =
            paymentRef.id;
        const merchantReference = `RT-CREDIT-${paymentId.slice(0, 8).toUpperCase()}`;
        const reasonForPayment = `Runtime Credit top-up - USD ${roundedAmount.toFixed(2)}`;
        const now = new Date().toISOString();
        ...
        if (!response.ok) {
            console.error('PesePay wallet top-up initiation failed:', response.status, responseBody);
            await markInitiationFailed(paymentId, null, null, `PesePay initiation failed with HTTP ${response.status}.`);
            return res.status(502).json({
                success: false,
                message: 'PesePay could not initiate the Runtime Credit top-up.',
            });
        }
        if (!responseBody ||
            typeof responseBody.payload !==
                'string' ||
            !responseBody.payload) {
            await markInitiationFailed(paymentId, null, null, 'PesePay returned an invalid response.');
            return res.status(502).json({
                success: false,
                message: 'PesePay returned an invalid response.',
            });
        }
        const providerTransaction = decryptPayload(responseBody.payload, encryptionKey);
        if (!providerTransaction.referenceNumber) {
            await markInitiationFailed(paymentId, null, null, 'PesePay response did not include a reference number.');
            return res.status(502).json({
                success: false,
        ...
        await paymentRef.set({
            provider_reference: providerTransaction.referenceNumber,
            transaction_id: providerTransaction.internalReference ||
                providerTransaction.referenceNumber,
            provider_status: providerTransaction.transactionStatus ||
                'INITIATED',
            provider_status_description: providerTransaction.transactionStatusDescription ||
                '',
            pesepay_poll_url: providerTransaction.pollUrl ||
                '',
            redirect_url: providerTransaction.redirectUrl ||
                '',
            updated_at: new Date().toISOString(),
        }, { merge: true });
        return res.json({
            ...
            transaction: {
                ...
                redirectRequired: Boolean(providerTransaction.redirectRequired),
                redirectUrl: providerTransaction.redirectUrl ||
                    null,
                pollUrl: providerTransaction.pollUrl ||
                    null,
        ...
        console.error('PesePay wallet top-up initiation error:', error);
        if (paymentId) {
            try {
                await markInitiationFailed(paymentId, null, null, 'Unable to initiate PesePay Runtime Credit top-up.');
            }
            catch (cleanupError) {
                console.error('Unable to clean up failed wallet top-up initiation:', cleanupError);
            }
        }
        return res.status(500).json({
            success: false,
            message: 'Unable to initiate PesePay Runtime Credit top-up.',
        });
    }
});
/*
