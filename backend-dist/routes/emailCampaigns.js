...
const escapeHtml = (value) => String(value)
    ...
    .replace(/"/g, '&quot;')
    .replace(/'/g, '&#039;');
const paragraphHtml = (value) => value
    .split(/\n{2,}/)
    .map((paragraph) => `<p style="margin:0 0 16px;font-size:14px;line-height:1.75;color:#52525b;">${escapeHtml(paragraph.trim()).replace(/\n/g, '<br>')}</p>`)
    .join('');
const buildCampaignHtml = ({ name, title, message, ctaLabel, ctaUrl, }) => {
...
const campaignCounts = async (campaignId) => {
...
router.get('/', authenticate, requireSuperAdmin, async (_req, res) => {
    try {
        ...
        const campaigns = await Promise.all(snapshot.docs.map(async (doc) => ({
            id: doc.id,
            ...doc.data(),
            counts: await campaignCounts(doc.id),
        })));
 ...
 */
router.get('/customers/:userId/domains', authenticate, requireSuperAdmin, async (req, res) => {
    try {
...
router.get('/customer-email/history', authenticate, requireSuperAdmin, async (req, res) => {
    try {
        const userId = String(req.query.userId || '').trim();
        let query = adminDb.collection('customer_email_history');
        if (userId)
...
router.post('/customer-email/send', authenticate, requireSuperAdmin, async (req, res) => {
    try {
        ...
        const requestedDomainIds = Array.isArray(req.body?.domainIds)
            ? [...new Set(req.body.domainIds
        ...
        for (const domainId of requestedDomainIds) {
            ...
            const belongsToCustomer = String(domain.user_id || '') === userId ||
...
router.get('/:campaignId/recipients', authenticate, requireSuperAdmin, async (req, res) => {
    try {
        ...
        return res
            ...
            .json({
            success: false,
            message: 'Unable to load campaign recipients.',
        });
    }
});
...
router.post('/:campaignId/pause', authenticate, requireSuperAdmin, async (req, res) => {
    try {
        await adminDb
            .collection('email_campaigns')
            .doc(req.params.campaignId)
        ...
        return res
            ...
            .json({
            success: false,
            message: 'Unable to pause campaign.',
        });
    }
});
router.post('/:campaignId/retry-failed', authenticate, requireSuperAdmin, async (req, res) => {
    try {
        const snapshot = await adminDb
            .collection('email_campaign_recipients')
            .where('campaign_id', '==', req.params.campaignId)
        ...
        return res
            ...
            .json({
            success: false,
            message: 'Unable to retry failed recipients.',
        });
    }
});
router.post('/:campaignId/process', authenticate, requireSuperAdmin, async (req, res) => {
    try {
        const campaignRef = adminDb
            .collection('email_campaigns')
            .doc(req.params.campaignId);
        ...
        const requestedBatch = Number(req.body?.batchSize ||
            5);
        const batchSize = Math.max(1, Math.min(20, Number.isFinite(requestedBatch)
        ...
        const recipientsSnapshot = await adminDb
            ...
            .where('campaign_id', '==', req.params.campaignId)
            .get();
        const queued = recipientsSnapshot.docs
            .filter((doc) => doc.data()
            ?.status ===
            'queued')
            .slice(0, batchSize);
        let sent = 0;
        let failed = 0;
        for (const recipientDoc of queued) {
            const recipient = recipientDoc.data();
            const attempts = Number(recipient.attempts ||
                0) + 1;
            try {
                await sendMail({
        ...
        const counts = await campaignCounts(req.params.campaignId);
        ...
        console.error('Process campaign failed:', error);
        return res
            .status(500)
            .json({
            success: false,
