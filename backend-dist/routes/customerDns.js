...
function input(body, domain) { const type = s(body?.type).toUpperCase(); if (!TYPES.has(type))
    throw new Error('Supported record types are A, AAAA, CNAME, MX, TXT and CAA.'); const content = s(body?.content); if (!content)
    throw new Error('Record content is required.'); const raw = Number(body?.ttl ?? 1); const o = { type, name: name(body?.name, domain), content, ttl: raw === 1 ? 1 : Math.max(60, Math.min(86400, Math.floor(raw || 1))) }; o.proxied = PROXY.has(type) ? Boolean(body?.proxied) : false; if (type === 'MX') {
