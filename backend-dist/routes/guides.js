...
const ensureBuiltinGuides = async () => {
    ...
    await Promise.all(BUILTIN_GUIDES.map(async (g) => {
        const ref = collection.doc(g.slug);
...
const isAdmin = (req) => req.runtimeUser?.role === 'super_admin';
const requireAdmin = (req, res, next) => isAdmin(req) ? next() : res.status(403).json({ success: false, message: 'Super administrator permission required.' });
...
router.get('/', async (req, res) => {
    try {
        ...
        const guides = snap.docs.map(serialize).sort((a, b) => String(b.published_at || b.updated_at || '').localeCompare(String(a.published_at || a.updated_at || '')));
...
router.get('/admin/all', authenticate, requireAdmin, async (_req, res) => {
    try {
        ...
        const guides = snap.docs.map(serialize).sort((a, b) => String(b.updated_at || '').localeCompare(String(a.updated_at || '')));
        res.json({ success: true, guides });
    }
    catch (error) {
        res.status(500).json({ success: false, message: error.message });
...
router.post('/:slug/view', async (req, res) => {
    try {
        await ensureBuiltinGuides();
        const ref = collection.doc(slugify(req.params.slug));
        const result = await adminDb.runTransaction(async (tx) => { const doc = await tx.get(ref); if (!doc.exists || doc.data()?.status !== 'published')
            throw new Error('Guide not found.'); const views = Number(doc.data()?.views || 0) + 1; tx.update(ref, { views }); return views; });
        res.json({ success: true, views: result });
    }
    catch (error) {
        res.status(error.message === 'Guide not found.' ? 404 : 500).json({ success: false, message: error.message });
    }
});
router.post('/:slug/useful', async (req, res) => {
    try {
        ...
        const ref = collection.doc(slugify(req.params.slug));
        const result = await adminDb.runTransaction(async (tx) => { const doc = await tx.get(ref); if (!doc.exists || doc.data()?.status !== 'published')
            throw new Error('Guide not found.'); const useful_count = Number(doc.data()?.useful_count || 0) + 1; tx.update(ref, { useful_count }); return useful_count; });
...
router.delete('/admin/:slug', authenticate, requireAdmin, async (req, res) => {
    try {
        await collection.doc(slugify(req.params.slug)).delete();
...
router.get('/render/:slug', async (req, res) => {
    try {
        ...
        const data = doc.exists && doc.data()?.status === 'published' ? doc.data() : null;
