...
const AUTO_DELETE_AFTER_DAYS =
  7;

const ABANDONED_ORDER_STATUSES =
  new Set([
    'pending',
    'unpaid',
    'payment_pending',
...
const ABANDONED_PAYMENT_STATUSES =
  new Set([
    'pending',
...
const refundRuntimeCredit =
  async (
    ...
    if (!userId) {
      throw new Error(
        `Cannot refund Runtime Credit for ${orderId}: user is missing.`
      );
    }
        ...
        transaction.create(
          refundLedgerRef,
          {
            ...
            reference_id:
              orderId,
            description:
              `Runtime Credit returned after abandoned order ${String(
                order.reference ||
                orderId
              )} was cancelled/removed.`,
            idempotency_key:
              `abandoned-order-refund:${orderId}`,
            created_at:
              now,
          }
        );
...
export type CleanupOrderResult = {
  cleaned: boolean;
  orderId: string;
  deletedPayments: number;
  deletedPendingDomains: number;
  refundedRuntimeCredit: number;
...
export const cleanupOrder =
  async (
    ...
    if (
      ...
      return {
        ...
        refundedRuntimeCredit:
          0,
        skippedReason:
          'Paid/completed orders are never abandoned-order cleanup targets.',
      };
    }
    ...
    if (
      ...
      return {
        ...
        refundedRuntimeCredit:
          0,
        skippedReason:
          'Only cancelled orders can be manually deleted.',
      };
    }
    ...
    if (
      ...
      return {
        ...
        refundedRuntimeCredit:
          0,
        skippedReason:
          `Order status ${status} is not an abandoned unpaid state.`,
      };
    }
    ...
    if (
      ...
      return {
        ...
        refundedRuntimeCredit:
          0,
        skippedReason:
          'Order has verified external money and requires manual financial review.',
      };
    }
    ...
    const refundedRuntimeCredit =
      await refundRuntimeCredit(
        orderId,
        order,
        verifiedCredit
      );
     ...
     */
    let deletedPendingDomains =
      0;

    for (
      const domainDoc
      of domainSnapshot.docs
    ) {
      const domain =
        domainDoc.data();
...
export const cleanupWalletTopup =
  async (
    paymentId: string
  ) => {
    const ref =
    ...
    if (
      ...
      return {
        cleaned:
          false,
        reason:
          'This payment is not a Runtime Credit top-up.',
      };
    }
    ...
    if (
      ...
      return {
        cleaned:
          false,
        reason:
          'Verified wallet top-ups cannot be deleted because credit was already added.',
      };
    }
...
const cleanupOrphanRecords =
  async () => {
    ...
    let orphanPaymentsDeleted =
      0;

    for (
      const paymentDoc
      of paymentSnapshot.docs
    ) {
      const payment =
        paymentDoc.data();
...
export const runAbandonedCleanup =
  async () => {
    const now =
      Date.now();
