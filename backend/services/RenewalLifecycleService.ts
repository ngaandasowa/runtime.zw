...
export type RenewalLifecycleRunResult = {
...
const eventAlreadyDone = (
  domain: FirebaseFirestore.DocumentData,
  milestone: Milestone
) =>
  Boolean(
    domain.renewal_lifecycle?.events?.[milestone]
      ?.completed_at
  );

const markEvent = async (
  domainRef: FirebaseFirestore.DocumentReference,
  milestone: Milestone,
  simulatedDate: Date,
...
const createRenewalOrder = async (
  ...
  const yearlyRate =
    Number(domain.renewal_price || 0);

  if (
    !Number.isFinite(yearlyRate) ||
    yearlyRate <= 0
  ) {
    throw new Error(
      `Renewal pricing is unavailable for ${domain.domain_name}.`
    );
  }
  ...
  const order = {
    id: orderId,
    user_id:
      String(domain.user_id || ''),
    user_email:
      String(domain.user_email || ''),
    reference,
    subtotal: yearlyRate,
    discount: 0,
    total: yearlyRate,
    currency:
      String(domain.currency || 'USD'),
    status: 'pending',
    purpose: 'domain_renewal',
    domain_id: domainRef.id,
    renewal_years: 1,
    lifecycle_generated: true,
    lifecycle_expiry_date:
      expiresAt,
    created_at: now,
    updated_at: now,
    items: [
      {
        ...
        description:
          `Domain Renewal: ${domain.domain_name} (1 Year @ $${yearlyRate.toFixed(2)}/yr)`,
        quantity: 1,
...
export class RenewalLifecycleService {
  async runProduction(): Promise<RenewalLifecycleRunResult> {
    return this.run(
      new Date().toISOString()
    );
  }
  ...
  async run(
    simulatedDateInput: string
  ): Promise<RenewalLifecycleRunResult> {
    const simulatedDate =
      dateOnlyUtc(
        simulatedDateInput
      );

    const domainsSnapshot =
      await adminDb
        .collection('domains')
        .get();
      ...
      if (milestone === 'dplus7') {
        ...
        results.push({
          ...
          orderId:
            renewal.orderId,
          action:
            'grace_period_ended_and_notified',
        });
      }
    }
...
export const renewalLifecycleService =
  new RenewalLifecycleService();
