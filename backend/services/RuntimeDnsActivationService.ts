...
export type RuntimeDnsActivationRunResult = {
  scanned: number;
  checked: number;
  activated: number;
  failed: number;
...
class RuntimeDnsActivationService {
  ...
  async run(): Promise<RuntimeDnsActivationRunResult> {
    if (this.running) {
      return {
        scanned: 0,
        checked: 0,
...
export const runtimeDnsActivationService =
  new RuntimeDnsActivationService();
...
export const startRuntimeDnsActivationScheduler =
  () => {
    ...
    const runSafely = async () => {
      try {
        const result =
          await runtimeDnsActivationService
            .run();

        console.log(
          `[Runtime DNS] activation check: scanned=${result.scanned}, checked=${result.checked}, activated=${result.activated}, failed=${result.failed}`
        );
      } catch (error) {
        console.error(
          '[Runtime DNS] activation scheduler failed:',
          error
        );
      }
    };
    ...
    console.log(
      `[Runtime DNS] activation scheduler started (${minutes} minute interval).`
    );
  };
