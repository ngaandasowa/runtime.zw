...
export type ApplyRuntimeCreditResult = {
  paymentId: string;
...
export const applyRuntimeCreditToOrder =
  async ({
    orderId,
    userId,
    actor,
    requestedAmount,
  }: ApplyRuntimeCreditInput):
    Promise<ApplyRuntimeCreditResult> => {
    const cleanOrderId =
      String(orderId || '').trim();
    ...
    const idempotencyKey =
      `order-runtime-credit:${cleanOrderId}`;
    ...
    const result =
      ...
      async (transaction) => {
        ...
        if (
          ...
          throw new Error(
            'You may only use Runtime Credit on your own order.'
          );
        }
        ...
        if (
          ...
          throw new Error(
            'This order has an invalid total.'
          );
        }
        ...
        if (
          ...
          throw new Error(
            'You do not have Runtime Credit available.'
          );
        }
        ...
        if (
          ...
          throw new Error(
            'No Runtime Credit can be applied to this order.'
          );
        }
        ...
        transaction.create(
          ledgerRef,
          {
            ...
            reference_id:
              cleanOrderId,
            description:
              `Runtime Credit applied to order ${String(
                order.reference ||
                  cleanOrderId
              )}`,
            idempotency_key:
    ...
    if (!result.alreadyApplied) {
      try {
        ...
        console.error(
          'Runtime Credit application email failed:',
          error
        );
      }
    }
