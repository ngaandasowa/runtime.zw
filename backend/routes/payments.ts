...
type PesePayTransaction = {
  ...
  /*
   * Some seamless methods can return provider-specific payment metadata
   * (for example a payment code or QR value). These fields are optional
   * because they are not present for every PesePay method/response.
   */
  metadataCode?: string;
  code?: string;
...
const normalisePesePayMethod = (
   ...
   */
  const seamlessMobileMoney =
    isEcoCash || isInnBucks || isOmari;
    ...
    try {
      ...
      if (
        ...
        return res.status(400).json({
          success: false,
          message:
            'A valid domain update is required.',
        });
      }
      ...
      const result =
        await adminDb.runTransaction(
          async (transaction) => {
            ...
            let description =
              'Domain details update requested.';
            ...
            if (
              ...
              if (
                ...
                const error =
                  new Error(
                    'Valid owner details are required.'
                  ) as Error & {
                    statusCode?: number;
                  };
    ...
    try {
      ...
      if (amount <= 0) {
        return res.status(400).json({
          success: false,
          message:
            'This order is already fully funded.',
        });
      }
  ...
  async (
    ...
    try {
      ...
      const amount = Math.round(
        (Number(req.body?.amount) + Number.EPSILON) * 100
      ) / 100;
      const paymentMethodCode =
        typeof req.body?.paymentMethodCode === 'string'
          ? req.body.paymentMethodCode.trim()
          : '';
      const customerPhoneNumber =
        typeof req.body?.customerPhoneNumber === 'string'
      ...
      return res.json({
        ...
        transaction: {
          ...
          metadataQrCode:
            ...
            null,
          transactionMetadata:
            transaction.transactionMetadata ?? transaction.metadata ?? null,
        },
      });
    } catch (error) {
...
router.get(
  ...
  async (
    ...
    try {
      ...
      const [orderDoc, paymentSnapshot] =
        await Promise.all([
          orderRef.get(),
          adminDb
            .collection('payments')
      ...
      const ownsOrder =
        order.user_id === runtimeUser.uid;
      const isAdmin =
        runtimeUser.role === 'super_admin';

      if (!ownsOrder && !isAdmin) {
        return res.status(403).json({
          success: false,
          message:
            'You may only view payments for your own order.',
        });
      }
      ...
      const payments = paymentSnapshot.docs.map(
        (doc) => ({
          id: doc.id,
          ...doc.data(),
        })
      ...
      const runtimeCreditApplied = money(
        verifiedPayments
          .filter(
            (payment: any) =>
      ...
      const runtimeCreditBalance = money(
        Number(
          walletDoc.exists
      ...
      return res.json({
        ...
        order: {
          id: orderId,
          reference:
            String(order.reference || orderId),
          total: orderTotal,
          currency:
      ...
      return res.status(500).json({
        success: false,
        message:
          'Unable to load the order payment summary.',
      });
    }
  }
);
    ...
    try {
      ...
      const result =
        await applyRuntimeCreditToOrder({
          ...
          actor:
            runtimeUser.email || runtimeUser.uid,
          requestedAmount,
        });
