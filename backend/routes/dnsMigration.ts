...
router.post('/:domainId/start', async(req:R,res)=>{
  try {
    ...
    const current=(x.d.nameservers||[]).map((n:any)=>s(n).toLowerCase()).filter(Boolean);
    ...
    if(assigned.length<2) throw new Error('Cloudflare did not return assigned nameservers.');
