...
const normalizeNs = (value: unknown): string[] =>
  Array.isArray(value)
    ? value.map((n) => s(n).toLowerCase().replace(/\.$/, '')).filter(Boolean)
    : [];

const sameNsSet = (left: unknown, right: unknown) => {
  const a = [...normalizeNs(left)].sort();
  const b = [...normalizeNs(right)].sort();
  return a.length === b.length && a.every((value, index) => value === b[index]);
};

const isZispaDomain = (domain: any) =>
  s(domain.processing_type).toLowerCase() === 'zispa' ||
  s(domain.domain_name).toLowerCase().endsWith('.co.zw');

async function resolveNameservers(nameservers: string[]) {
  const dns = await import('node:dns/promises');
  const output: Array<{ hostname: string; ip: string }> = [];
  for (const hostname of nameservers) {
    let addresses: string[] = [];
    try { addresses = await dns.resolve4(hostname); } catch {}
    if (!addresses.length) {
      try { addresses = await dns.resolve6(hostname); } catch {}
    }
...
type R = Request & { runtimeUser?: { uid: string; email?: string } };

async function owned(req:R,res:Response) {
  ...
  if(!['active','expired'].includes(s(d.status))){
    res.status(409).json({success:false,message:'Runtime DNS migration is available after domain registration.'});return null;
  }
  return {id,ref,d};
}

function migrationView(d:any) {
...
router.post('/:domainId/start', async(req:R,res)=>{
  try {
    const x=await owned(req,res); if(!x)return;
    const migrationStatus=s(x.d.dns_migration?.status).toLowerCase();
    const assigned=normalizeNs(
      x.d.dns_migration?.pending_nameservers?.length
        ? x.d.dns_migration.pending_nameservers
        : x.d.cloudflare?.assigned_nameservers
    );
    const genuinelyActive =
      s(x.d.dns_provider).toLowerCase()==='cloudflare' &&
    ...
    if(current.length<2) return res.status(409).json({success:false,message:'At least two current nameservers are required before migration.'});
    ...
    if(zoneAssigned.length<2) throw new Error('Cloudflare did not return assigned nameservers.');
...
router.post('/:domainId/refresh', async(req:R,res)=>{
  try {
    const x=await owned(req,res); if(!x)return;
    const zoneId=s(x.d.dns_migration?.cloudflare_zone_id);
...
router.post('/:domainId/review', async(req:R,res)=>{
  try {
    const x=await owned(req,res); if(!x)return;
    const zoneId=s(x.d.dns_migration?.cloudflare_zone_id);
    if(!zoneId)return res.status(409).json({success:false,message:'Start the migration first.'});
    const discovered=await cloudflareDnsService.listScannedDnsRecords(zoneId);
    const selected=new Set(Array.isArray(req.body?.selectedKeys)?req.body.selectedKeys.map((v:any)=>s(v)):[]);
    if(!selected.size)return res.status(400).json({success:false,message:'Select at least one DNS record to import.'});

    const key=(r:any)=>[s(r.type).toUpperCase(),s(r.name).toLowerCase(),s(r.content),String(r.priority??'')].join('|');
    const safe=(r:any)=>{
      const o:any={type:s(r.type).toUpperCase(),name:s(r.name),content:s(r.content),ttl:Number(r.ttl)||1};
      if(typeof r.proxied==='boolean')o.proxied=r.proxied;
