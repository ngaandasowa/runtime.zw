...
} from '../services/CloudflareDnsService.js';
...
router.post(
  ...
  async (
    ...
    try {
      ...
      if (
        ...
        return res
          ...
          .json({
            success: false,
            message:
              'Provide domainId or domainName.',
          });
      }
      ...
      if (
        isActiveLegacyDomain
      ) {
        return res
          .status(409)
          .json({
            success: false,
            protectedLegacyDomain:
              true,
            message:
              'This active domain already uses existing nameservers. Runtime will not automatically migrate it to Cloudflare.',
            currentNameservers:
              existingNameservers,
          });
      }
      ...
      if (
        ...
        throw new Error(
          'Cloudflare did not return the expected nameservers.'
        );
      }
      ...
      return res.json({
        ...
        zoneStatus:
          ...
          'pending',
        nameservers,
        runtimeDomainUpdated:
          Boolean(
            domainRef
          ),
        message:
          'Cloudflare zone is provisioned. Registry nameservers were not changed by this test.',
      });
    } catch (error) {
      console.error(
        'Cloudflare test provisioning failed:',
        error
      );
...
router.post(
  ...
  async (
    ...
    try {
      ...
      const domainId =
        typeof req.body?.domainId === 'string'
      ...
      const ref =
        adminDb.collection('domains').doc(domainId);
       ...
       */
      const zone =
        await cloudflareDnsService.listZone(domainName);
      ...
      if (!zone) {
        return res.status(409).json({
          ...
          code: 'ZONE_NOT_FOUND',
          message:
            'No Runtime Cloudflare zone exists for this domain. Nothing was changed.',
        });
      }

      const zoneStatus =
        String(zone.status || '').trim().toLowerCase();
      ...
      if (zoneStatus !== 'active') {
        return res.status(409).json({
          ...
          zoneStatus,
          message:
            `Cloudflare zone is ${zoneStatus || 'unknown'}, not Active. Nothing was changed.`,
        });
      }
      ...
      if (assignedNameservers.length < 2) {
        return res.status(409).json({
          ...
          message:
            'Cloudflare did not return the expected assigned nameservers. Nothing was changed.',
        });
      }
      ...
      try {
        ...
        return res.status(409).json({
          ...
          message:
            'Unable to verify the domain’s public authoritative nameservers. Nothing was changed.',
        });
      }
      ...
      if (!delegationMatches) {
        return res.status(409).json({
          ...
          authoritativeNameservers,
          message:
            'Cloudflare is Active, but the domain’s public authoritative nameservers do not match this Runtime zone. Nothing was changed.',
        });
      }
      ...
      const migration =
        ...
        typeof domain.dns_migration === 'object'
          ? {
              ...
              status: 'completed',
              completed_at:
                domain.dns_migration.completed_at || now,
              reconciled_at: now,
              reconciled_by:
      ...
      await ref.set(
        {
          ...
          dns_reconciliation: {
            ...
            reconciled_by:
              ...
              req.runtimeUser?.uid ||
              'super_admin',
            previous_dns_provider:
              domain.dns_provider || null,
            previous_dns_status:
              domain.dns_status || null,
            verified_zone_id: zone.id,
            verified_nameservers:
              authoritativeNameservers,
          },
          updated_at: now,
        },
          ...
          .set(
            {
              ...
              registry_response_notes:
                'Runtime DNS state reconciled after Cloudflare Active status and public authoritative nameservers were independently verified.',
              updated_at: now,
            },
            { merge: true }
          );
      ...
      return res.json({
        ...
        nameservers: assignedNameservers,
        message:
          'Runtime DNS state reconciled successfully. Cloudflare is Active and the public delegation matches the Runtime zone.',
      });
    } catch (error) {
      console.error(
        'Runtime DNS reconciliation failed:',
        error
      );
