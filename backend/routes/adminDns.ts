...
router.use((req: Request, res: Response, next) => {
  if ((req as RuntimeAuthenticatedRequest).runtimeUser?.role !== 'super_admin') {
...
function actor(req: Request) {
  const user = (req as RuntimeAuthenticatedRequest).runtimeUser;
  return { actorUid: s(user?.uid), actorEmail: s(user?.email), actorRole: 'admin' as const };
}

router.get('/:domainId', async (req, res) => {
  try {
    const x = await domainFor(req, res); if (!x) return;
